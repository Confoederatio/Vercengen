...
ve.UndoRedo = class extends ve.Component {
				...
				components_obj: {
					file_actions_bar: veRawInterface({
						...
						do_not_overwrite_on_load: veToggle(settings_obj.do_not_overwrite_on_load, {
							name: loc("ve.registry.localisation.UndoRedo_do_not_overwrite_on_load"),
							tooltip: loc("ve.registry.localisation.UndoRedo_do_not_overwrite_on_load_tooltip"),
							onuserchange: (v) => {
	 ...
	 * @memberof ve.Component.ve.UndoRedo
	 * 
	 * @param {boolean} [arg0_force_update] - Whether to force update the current Actions Map.
	 */
	draw (arg0_force_update) {
		...
		//Render canvas
		{
			...
			Object.iterate(timeline_graph, (local_key, local_value) => {
				...
				//Selection logic: check if current index falls within the action domain (inclusive)
				let is_selected = false;
				if (
					local_value.timeline_id === DALS.Timeline.current_timeline &&
					DALS.Timeline.current_index >= local_value.options.domain[0] &&
					DALS.Timeline.current_index <= local_value.options.domain[1]
				)
					is_selected = true;
				...
				node_positions[local_key] = {
					...
					domain: local_value.options.domain,
			...
			//6. Corrected Click Handler: jump precisely to the last action of the group
			this.canvas_el.onclick = (e) => {
				...
				Object.iterate(node_positions, (local_key, local_value) => {
					if (
						...
						//Jump to the end of the clicked group's domain
						local_timeline.jumpToAction(local_value.domain[1]);
		...
		//1. Render HTML
		{
			...
			if (!(skip_html_redraw && skip_timeline_redraw) || force_update) {
					...
					filter_button: new ve.Button(() => {
						let local_context_menu = new ve.ContextMenu({
							radio_select: new ve.Radio({
								alphabetical_ascending: { name: loc("ve.registry.localisation.UndoRedo_alphabetical_ascending") },
								alphabetical_descending: { name: loc("ve.registry.localisation.UndoRedo_alphabetical_descending") },
								chronological_ascending: { name: loc("ve.registry.localisation.UndoRedo_chronological_ascending") },
								chronological_descending: { name: loc("ve.registry.localisation.UndoRedo_chronological_descending") },
								last_modified: { name: loc("ve.registry.localisation.UndoRedo_last_modified") },
								least_actions: { name: loc("ve.registry.localisation.UndoRedo_least_actions") },
								most_actions: { name: loc("ve.registry.localisation.UndoRedo_most_actions") }
							}, {
				...
				for (let i = 0; i < timeline_groups.length; i++) {
					...
					let local_name = timeline_groups[i][0].name || `New Action`;
					
					//Corrected selection logic for HTML list
					...
					let header_el = new ve.RawInterface({
						action_name: new ve.HTML(`${local_name} (${String.formatNumber(timeline_groups[i].length)})&nbsp;&nbsp;<br>#${local_domain[0]} - ${local_domain[1]}`, {
							attributes: { "data-is-selected": is_selected },
							style: { display: "inline-block" }
						}),
						jump_to_button: new ve.Button(() => {
							timeline_obj.jumpToAction(local_domain[1]);
							this.fireToBinding();
						}, {
							name: `<icon>arrow_right_alt</icon>`,
							tooltip: loc("ve.registry.localisation.UndoRedo_jump_to"),
							style: { "button": { marginLeft: "var(--padding)" }, marginLeft: "auto", order: 99 }
						}),
						branch: new ve.Button(() => {
							//Branch at exactly the point reached after jump
							timeline_obj.jumpToAction(local_domain[1]).then(() => {
		...
		this.current_index = DALS.Timeline.current_index;
		this.current_timeline = DALS.Timeline.current_timeline;
	}
